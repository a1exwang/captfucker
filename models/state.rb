class State
  class PartOfState
    ...
    def initialize(graph, text, probability)
      self.graph = graph
      self.text = text
  ...
  class PossibleState
    ...
    def initialize(part_of_states, probability = 0)
      self.part_of_states = part_of_states
      self.text = ''
      part_of_states.each do |part|
        self.text += part.text
      end
      self.probability = probability
    end
    def +(other)
      PossibleState.new(part_of_states + other.possible_states)
    end
  end
